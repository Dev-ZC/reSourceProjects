...
let id = 0;
const getId = () => `textboxnode_${id++}`;

interface DraggableTextBoxNodeProps {
  className?: string;
  children: React.ReactNode;
  onDrop: (nodeId: string, position: XYPosition) => void;
}

export function DraggableTextBoxNode({ className, children, onDrop }: DraggableTextBoxNodeProps) {
...
interface DraggableTextBoxNodeButtonProps {
  onNodeAdded: (nodeId: string) => void;
}

export function DraggableTextBoxNodeButton({ onNodeAdded }: DraggableTextBoxNodeButtonProps) {
  ...
  const handleNodeDrop = useCallback(
    (nodeId: string, screenPosition: XYPosition) => {
      ...
      // Create a new text box node and add it to the flow
      if (isInFlow) {
